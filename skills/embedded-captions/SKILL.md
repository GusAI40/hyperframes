...
- **Optional cloud matte (higher quality):** `EC_MATTE=cloud` routes matting to the HeyGen CLI's `background-removal` (Bria — the avatar-pipeline model; sharper edges + fewer furniture leaks than local u2net). Needs the `heygen background-removal` command (codegen'd from OpenAPI PR #40076 — **not shipped yet**) + a HeyGen API key (`$HEYGEN_API_KEY`, or `heygen auth login` / `hyperframes auth login`). `matte-cloud.cjs` produces the same `frames_fg/`; **any failure (command absent / no key / API error) falls back to the local engine** — safe to set today (it no-ops to local until the command ships).
