...
//      one Read. In w2h, there is no detached BGM step — `bgm_status.json`
//      is never present, so `bgm.status` resolves to "disabled" via the
//      defensive read fallback. If w2h adopts detached BGM later, port
//      plv2's wait-bgm.mjs verbatim — its exit-0 / status-file contract
//      maps cleanly onto this consumer.
//      `preflight_clean = gates_clean && caption_keepout has 0 violations
//      && perception_clean` — the agent uses this for fast-path decision.
//
// Exit codes:
...
// --cli "<full command>" or env HYPERFRAMES_CLI lets the caller pin which
// hyperframes CLI binary the gates spawn. Without it, falls back to
// `npx --yes hyperframes@<version>` (the published-package path). When the
// worktree has unreleased fixes (lint rules, perception gate, etc.) you want
// the local source instead, e.g.
//   --cli "npx tsx /path/to/packages/cli/src/cli.ts"
// or `HYPERFRAMES_CLI="npx tsx /path/to/packages/cli/src/cli.ts"`.
const cliOverride = flag("cli", null) || process.env.HYPERFRAMES_CLI || null;
...
let npxPrefix;
let spawnBase; // [cmd, ...prefixArgs] — what spawnSync receives before per-gate args

if (cliOverride) {
  // Caller pinned the CLI. Tokenize the override string on whitespace; first
  // token is the executable, the rest are leading args (e.g. `tsx /repo/.../cli.ts`).
  const tokens = cliOverride.trim().split(/\s+/).filter(Boolean);
  if (tokens.length === 0) {
    console.error("✗ preflight-finalize: --cli was empty");
    process.exit(1);
  }
  spawnBase = tokens;
  npxPrefix = cliOverride;
  pinnedVersion = "(custom --cli override)";
} else {
  ...
  npxPrefix = `npx --yes hyperframes@${pinnedVersion}`;
  spawnBase = ["npx", "--yes", `hyperframes@${pinnedVersion}`];
}
...
// spawnSync (not execSync) with array args — no shell, so no metachar injection
// even if the --cli override contains spaces or quotes.
let cliVersionLine = "";
try {
  const [verCmd, ...verPrefix] = spawnBase;
  const verRes = spawnSync(verCmd, [...verPrefix, "--version"], {
    cwd: hyperframesDir,
    encoding: "utf8",
    stdio: ["ignore", "pipe", "pipe"],
    timeout: 60_000,
  });
  cliVersionLine = (verRes.status === 0 ? verRes.stdout : verRes.stderr || "").trim() ||
    `(exit ${verRes.status})`;
} catch (e) {
  cliVersionLine = `(failed to resolve ${cliOverride ? "--cli override" : `hyperframes@${pinnedVersion}`}: ${e.message?.split("\n")[0] || "unknown"})`;
}
...
function runGate(name, args, { timeoutMs = 90_000 } = {}) {
  const t0 = Date.now();
  const [cmd, ...prefixArgs] = spawnBase;
...
// scene is long (>= 8s). plv2's effect-list / creative-brief heuristics were
// stripped during the w2h port — w2h-prep.mjs does not emit `effects[]` or
// `creative_brief`, so the long-duration check is the only signal that
// applies here. If w2h later adds an effects registry, re-introduce the
// MULTI_ACT_EFFECTS Set + HIGH_RISK_BRIEF_RX regex from plv2 and OR them in.
...
for (const group of groupSpec.groups || []) {
  for (const sid of group.scene_ids || []) {
    ...
    pushTs(midpoint);
    // w2h-prep does not emit s.effects; preserved as empty array for brief schema parity (referenced at sceneRows.effects below).
    const effects = [];
    const highRisk = dur >= 8;
    const extras = [];
    if (highRisk) {
...
if (perception.skipped) {
  anomalies.push({
    ...
    actionable_alternative: `npm i puppeteer-core && ${cliOverride || "npx hyperframes"} browser install`,
  });
}
...
console.log(`  preflight_clean: ${preflightClean ? "yes (gates + caption keep-out + perception)" : "no"}`);
console.log(
...
if (perception.skipped) {
  ...
  console.log(
    `      enable with:  (cd "${hyperframesDir}" && npm i puppeteer)   # or:  npm i puppeteer-core  + install Chrome via \`${cliOverride || "npx hyperframes"} browser install\``,
  );
