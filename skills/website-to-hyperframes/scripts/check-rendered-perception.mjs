...
const PROBE = function probe(sid, compRel, cfg) {
  ...
  const selectorFor = (el) => {
    ...
    // w2h adaptation: accept plv2's s\d+- prefix AND w2h cell-A's b\d+- prefix
    // (e.g. b2-title, b3-cta). Cell-G unprefixed classes still fall through to
    // tag-only selector — known limitation, documented in beat-builder-guide.md.
    const cls = Array.from(el.classList).filter((c) => c && /^(?:s|b)\d+-/.test(c));
  ...
  // w2h adaptation: extends plv2's decorative vocabulary with tokens observed
  // in cell-A and cell-G fixtures (drifter / bloom / flash / chip / accent /
  // caret / cursor / arrow / underline / tile / spark / grad / highlight / ghost).
  // Word-boundary-anchored + combined with other heuristics — false-positive
  // rate stays low; false-negative is preferred to false-positive here.
  const DECO_RX =
    /\b(bg|background|dot-?grid|mesh|gradient|grad|swell|ambient|texture|noise|scanline|surface|overlay|halo|glow|bloom|flash|spark|frame|pin|corner-?pin|deco|star-?burst|burst|ring|stripe|rect|shadow|pulse|ripple|measure|probe|scrim|backdrop|veil|fog|grain|drifter|chip|accent|caret|cursor|arrow|underline|highlight|tile|ghost)\b/i;
  const isInDecor = (el) => {
    let p = el;
  ...
  // w2h adaptation: cell-A and cell-G beats don't always wrap their scene in
  // `<div id="root">`; the composition root is identified by [data-composition-id]
  // instead. Prefer that wrapper over document.body so bbox checks measure
  // against the actual scene area (1920×1080), not the body padding wrapper.
  const root =
