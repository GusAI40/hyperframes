...
async function runHtml(argv) {
  ...
  // --cli "<full command>" or HYPERFRAMES_CLI env var lets the caller pin the
  // hyperframes CLI binary used by `<cli> add <skin>`. Without it, falls back
  // to `npx hyperframes add`. Pin to local source when running off an
  // unreleased worktree: --cli "npx tsx /path/to/packages/cli/src/cli.ts"
  ...
  if (usePresetSkin) {
    ...
    if (skinFile) {
      ...
      // for offline/CI golden renders). When --cli or HYPERFRAMES_CLI is set,
      // tokenize the override (first token = executable, rest = leading args).
      const addBase = cliOverride
        ? cliOverride.trim().split(/\s+/).filter(Boolean)
        : ["npx", "hyperframes"];
      const [addCmd, ...addPrefix] = addBase;
      try {
        execFileSync(addCmd, [...addPrefix, "add", winner, "--no-clipboard"], {
          cwd: hyperframesDir,
          stdio: "pipe",
        });
      } catch (e) {
        die(
          `\`${addBase.join(" ")} add ${winner}\` failed (offline? pass --skin-file <path>): ${e.message}`,
        );
      }
