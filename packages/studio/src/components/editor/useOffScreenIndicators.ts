...
function indicatorsEqual(a: OffScreenIndicator[], b: OffScreenIndicator[]): boolean {
  ...
  for (let i = 0; i < a.length; i++) {
    const ai = a[i]!;
    const bi = b[i]!;
    if (ai.key !== bi.key || Math.abs(ai.left - bi.left) > 0.5 || Math.abs(ai.top - bi.top) > 0.5 || Math.abs(ai.width - bi.width) > 0.5 || Math.abs(ai.height - bi.height) > 0.5)
      return false;
  }
  return true;
}
  ...
  useMountEffect(() => {
    ...
    const update = () => {
      ...
      for (const el of targets) {
        ...
        next.push({ key, elementId: el.id || base, left: elLeft, top: elTop, width: elW, height: elH });
      }
