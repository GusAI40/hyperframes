...
import { readCachedMusic, musicDir, writeSearchCache } from "./state.js";
import { analyzeAudio, formatAnalysisLines, type AudioAnalysis } from "../sfx/analyze.js";
import { requireSfxKey } from "../sfx/auth.js";

export const examples: Example[] = [
  ["Download a track from a prior search", "hyperframes music add 8b1ac2f2e69edd9b"],
  ["Add into a specific project", "hyperframes music add 8b1ac2f2e69edd9b --dir ./my-video"],
];
...
export default defineCommand({
  meta: {
    name: "add",
    description: "Download a music track into the project's assets/music/ directory",
  },
  args: {
    id: {
      type: "positional",
      description: "The music id from `hyperframes music search`",
      required: true,
    },
  ...
  async run({ args }) {
    ...
    const entry = readCachedMusic(projectDir, args.id);
    if (!entry) {
      errorBox(
        "Unknown music id",
        `No cached result for "${args.id}".`,
        'Run `hyperframes music search "<description>"` first, then add by id.',
      );
      process.exit(1);
    }
    ...
    try {
      ...
      const dest = join(musicDir(projectDir), `${args.id}${extFromUrl(url)}`);
      ...
      // Measure the track so the agent can place + balance it without hearing it.
      ...
      console.log(c.dim("  Embed it as a bed (ducks under voiceover):"));
      console.log(
        `  ${c.dim(`<audio src="${rel}" data-start="0" data-duration="${dur}" data-volume="0.45"></audio>`)}`,
      );
      if (analysis) {
      ...
      reportApiError("Music download failed", err);
    }
  },
});
