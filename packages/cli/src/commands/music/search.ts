...
import { writeSearchCache, type CachedMusic } from "./state.js";
import { requireSfxKey } from "../sfx/auth.js";

export const examples: Example[] = [
  ["Search by mood", 'hyperframes music search "moody ambient pad"'],
  ["Upbeat reel bed", 'hyperframes music search "upbeat lo-fi reel bed"'],
  ["JSON output (for agents)", 'hyperframes music search "tense synth drone" --json'],
];

export default defineCommand({
  meta: {
    name: "search",
    description: "Semantic search over the HeyGen music catalog",
  },
  args: {
    query: {
      type: "positional",
      description: "Natural-language description of the music you want (mood / feel / genre)",
      required: true,
    },
  ...
  async run({ args }) {
    ...
    spin?.start(`Searching music for "${args.query}"...`);
    try {
      ...
      const now = Date.now();
      const items: CachedMusic[] = (res.data ?? []).map((it) => ({
        id: it.id,
        name: it.name,
      ...
      // Cache so `music add <id>` can resolve the (expiring) download URL.
      try {
      ...
      spin?.stop(
        c.success(
          `Found ${items.length} track${items.length === 1 ? "" : "s"} for "${args.query}"`,
        ),
      );
      if (items.length === 0) {
        console.log(c.dim("  No matches — try a broader mood, or lower --minScore."));
        return;
      }
      ...
      console.log(
        `\n  ${c.dim("Add one to your project:")}  ${c.accent("hyperframes music add <id>")}`,
      );
    } catch (err) {
      spin?.stop(c.error("Search failed"));
      reportApiError("Music search failed", err);
    }
  },
});
